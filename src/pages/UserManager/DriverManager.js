...
const NewAccount = Form.create()(props => {
  ...
  return (
    ...
    >
      ...
      <FormItem {...labelLayout} label="司机名称">
      ...
      {type === 'add' && selectRole.role_type === "APPLICATION_USER_CUSTOM" &&
        selectRole.is_init && (
...
@connect(({ user, weixin, loading, role }) => ({
  ...
  qrCodeLoading: loading.effects["weixin/fetchDriverQRCode"],
  data: user.list,
...
class ShopManager extends PureComponent {
  ...
  componentDidMount() {
    try {
      ...
      const driverRole = roles.find(item => {
        return item.role_type === "APPLICATION_USER_CUSTOM" && item.is_init;
      });
      this.setState({ selectRole: driverRole });
      this.driverRoleId = driverRole ? driverRole.id : undefined;
    } catch (error) {
      return;
    }
  ...
  handleAdd = fields => {
    ...
    const data = { ...fields, role_id: this.driverRoleId };
    dispatch({
  ...
  onQRCode = record => {
    ...
    dispatch({
      type: "weixin/fetchDriverQRCode",
      payload: {
        user_id: `driver_${record.user.id}`,
        onLoad: msg => {
          this.setState({
            loadingMessage: msg || ""
          });
